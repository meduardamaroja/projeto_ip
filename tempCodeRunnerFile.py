...
client = None
api_key_invalida = False
if GEMINI_API_KEY:
    try:
        client = genai.Client(api_key=GEMINI_API_KEY)
    except Exception as e:
        logging.exception("ERRO DE CONFIGURAÇÃO: Falha ao inicializar o cliente Gemini")
        # Verifica se é erro de chave vazada/inválida
        if hasattr(e, "args") and e.args and isinstance(e.args[0], dict):
            error_info = e.args[0]
            if (
                isinstance(error_info, dict)
                and "error" in error_info
                and error_info["error"].get("status") == "PERMISSION_DENIED"
            ):
                api_key_invalida = True
else:
    logging.warning("ALERTA: Variável GEMINI_API_KEY não configurada.")
...
def gemini():
    ...
    if api_key_invalida:
        resposta = (
            "Erro: Sua chave da API Gemini foi vazada ou está inválida.<br>"
            "Por favor, gere uma nova chave em <a href='https://aistudio.google.com/app/apikey' target='_blank'>Google AI Studio</a> "
            "e atualize o arquivo <b>.env</b>."
        )
    ...
    if pergunta:
        ...
        if client:
            try:
                ...
                logging.exception("Erro ao consultar a IA")
                # Verifica se é erro de chave vazada ou permissão negada
                if hasattr(e, "args") and e.args and isinstance(e.args[0], dict):
                    error_info = e.args[0]
                    if (
                        isinstance(error_info, dict)
                        ...
                        resposta = (
                            "Erro: Sua chave da API Gemini foi vazada ou está inválida.<br>"
                            "Por favor, gere uma nova chave em https://aistudio.google.com/app/apikey "
                            "e atualize o arquivo <b>.env</b>."
                        )
                    else:
